...
<div id="bodyWrapper">
	...
	<form action="/mvProject/moviePoint/reviewWrite.do" id=reviewregit method="post">
		...
		<div class="showMessageDiv">
			<div class="inputShowMessageDiv">
				...
				<textarea id=reviewcontents name="reviewcontents"  cols=97 rows=5
					...
					placeholder="운영원칙에 어긋나는 게시물로 판단되는 글은 제재 조치를 받을 수 있습니다.">${reviewcontents}</textarea>
			</div>
...
function fnChkByte(obj, maxByte){
	...
	function reviewSubmit(){
		if($('#reviewcontents').attr('value') == ""){
			...
			$.ajax({
		        type : "GET",
		        url : "/mvProject/moviePoint/modifyreviewcontents.do",
		        ...
		        success : function(result) {/* 
		           ...
		           if(result.result > 0){
		        	  if (confirm("평가를 수정하시겠습니까?")) {
		                     location.href="/mvProject/moviePoint/charmPoint.do?mv_seq="+$('#mv_seq').val()+
		        	  ...
		        	  location.href="/mvProject/moviePoint/moviePoint.do?mv_seq="+$('#mv_seq').val()+
              				   "&Movienameforkr="+$('#Movienameforkr').val()+
      						   "&page="+$('#page').val()
		           }
