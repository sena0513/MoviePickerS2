...
<script>
	...
	$("#modifybtn").click(makefunc("modifyqna.do?seq=${list.seq}&pg=${pg}"));
	$("#backbtn").click(function(){
		location.href = "qnalistform.do"
	});
	$("#deletebtn").click(function() {
		if (confirm("정말로 삭제하시겠습니까?")) {
			location.href = "deleteqna.do?seq=${list.seq}&pg=${pg}";
		} else {
			alert("취소하셨습니다");
			location.href = "qnalistform.do?pg=${pg}"
		}
	});
});
</script>
<form action="/mvProject/adminQnA/qnaregit.do" method="post" id=movepage>
	<jsp:include page="../admininclude/admintop.jsp" />
	
	<table border=1>
		<tr>
			<td>자주하는질문 제목<input type=hidden value=${list.seq} name=seq></td>
